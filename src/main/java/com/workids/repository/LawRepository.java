...
public interface LawRepository extends JpaRepository<Law, Long>, QuerydslPredicateExecutor<Law>{
     ...
     * 나라 법 조회
     * */
    @Query("select new com.workids.dto.response.ResponseLawDto (l.lawNum, l.content, l.fine, l.penalty, l.updateDate) from Law l where l.nation.nationNum = ?1 and l.state=0")
    List<ResponseLawDto> findByNation(Long nation_num);
    ...
    @Modifying
    int updateFineByLawNum(Long law_num, int fine);
    ...
    @Modifying
    int updateStateByLawNum(Long law_num);
